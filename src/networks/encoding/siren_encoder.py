...
class FixedAutoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = x.view(
            -1, 1, FixedAutoencoder.tile_size, FixedAutoencoder.tile_size
        )
        x = self.encoder(x)
        x = self.decoder(x)
        x = x.view(
...
def load_fixed_autoencoder(device, model_path):
    """Load the autoencoder model."""
    model = FixedAutoencoder()
...
class FixedEncoder(nn.Module):
    ...
    def __init__(self, model_path, device):
        ...
        autoencoder = load_fixed_autoencoder(device, model_path)
        self.encoder = autoencoder.encoder
