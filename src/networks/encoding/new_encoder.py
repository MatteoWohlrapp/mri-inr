...
# Legacy autoencoder
class HardcodedAutoencoder(nn.Module):
    ...
    Attributes:
        ...
        latent_dim (int): The dimension of the latent space.
    """
    # Hardcoded since only used for legacy purposes
    tile_size = 32
    latent_dim = 256

    def __init__(self, latent_dim = 256):
        super(HardcodedAutoencoder, self).__init__()
        self.encoder = nn.Sequential(
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: The output tensor.
        """
        x = x.view(-1, 1, HardcodedAutoencoder.tile_size, HardcodedAutoencoder.tile_size)
        x = self.encoder(x)
        x = self.decoder(x)
        x = x.view(-1, 1, HardcodedAutoencoder.tile_size, HardcodedAutoencoder.tile_size)
        return x
    
def load_hardcoded_autoencoder(device, model_path):
    """Load the autoencoder model."""
    model = HardcodedAutoencoder()
    model.load_state_dict(torch.load(model_path, map_location=device)['state_dict'])
    model.to(device)
    return model

class HardcodedEncoder(nn.Module):
    """Encoder for perceptual loss. Load the weights from the autoencoder."""
    ...
    def __init__(self, model_path, device):
        ...
        Args:
            ...
            img_size (int): The size of the input image.
        """
        super(HardcodedEncoder, self).__init__()
        autoencoder = load_hardcoded_autoencoder(device, model_path)
        self.encoder = autoencoder.encoder
